...
from locators.elements_page_locators import TextBoxPageLocators, CheckBoxPageLocators, RadioButtonLocators, \
    WebTablesLocators, ButtonsPageLocators, LinksPageLocators, FilesPageLocators, DynamicPropertiesPageLocators
...
class WebTablePage(BasePage):
    ...
    def select_up_to_some_rows(self):
        ...
        for x in count:
            ...
            self.go_to_elemenet(count_row_button)
...
class ButtonsPage(BasePage):
    ...
    def click_on_different_button(self, type_click):
        if type_click == 'double':
            self.action_double_click(self.is_visible(self.locators.DOUBLE_BUTTON))
            return self.check_clicked_on_the_button(self.locators.SUCCESS_DOUBLE)

        if type_click == 'right':
            self.action_right_click(self.is_visible(self.locators.RIGHT_CLICK_BUTTON))
...
class LinksPage(BasePage):
    ...
    def check_new_tab_simple_link(self):
        simple_link = self.is_visible(self.locators.SIMPLE_LINK)
    ...
    def check_broken_link(self, url):
        ...
        if request.status_code == 200:
            self.is_present(self.locators.BAD_REQUEST).click()
        else:
            return request.status_code

class WorkWithFiles(BasePage):
    locators = FilesPageLocators()

    def upload_file(self):
        file_name, path = generated_file()
        self.is_visible(self.locators.UPLOAD_FILE).send_keys(path)
        os.remove(path)
    ...
    def download_file(self):
        ...
        path_name_file = f'/home/oleg/PycharmProjects/AQA/filetest{random.randint(0, 99)}.jpg'
...
class DynamicPropertiesPage(BasePage):
    ...
    def check_button_appear(self):
        try:
            self.is_visible(self.locators.VISIBLE_AFTER_TIME)
        except TimeoutException:
            return False
        return True