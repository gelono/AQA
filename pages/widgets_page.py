...
class AutoCompletePage(BasePage):
    ...
    def remove_value_from_multi(self):
        count_value_before = len(self.are_present(self.locators.MULTI_VALUE))
        remove_button_list = self.are_visible(self.locators.MULTI_VALUE_REMOVE)
        for value in remove_button_list:
            value.click()
            break
        count_value_after = len(self.are_present(self.locators.MULTI_VALUE))
        return count_value_before, count_value_after

    def check_color_in_multi(self):
        color_list = self.are_present(self.locators.MULTI_VALUE)
        colors = []
...
class DatePickerPage(BasePage):
    ...
    def select_date_and_time(self):
        ...
        value_date_before = input_date.get_attribute('value')
        input_date.click()
        self.is_visible(self.locators.DATE_AND_TIME_MONTH).click()
        self.set_date_item_from_list(self.locators.DATE_AND_TIME_MONTH_LIST, date.month)
        self.is_visible(self.locators.DATE_AND_TIME_YEAR).click()
    ...
    def set_date_item_from_list(self, elements, value):
        item_list = self.are_present(elements)
...
class SliderPage(BasePage):
    ...
    def change_slider_value(self):
        value_before = self.is_visible(self.locators.SLIDER_VALUE).get_attribute('value')
        slider_input = self.is_visible(self.locators.INPUT_SLIDER)
        self.action_drag_and_drop_by_offset(slider_input, random.randint(1, 100), 0)
        value_after = self.is_visible(self.locators.SLIDER_VALUE).get_attribute('value')
...
class ProgressBarPage(BasePage):
    ...
    def change_progress_bar_value(self):
        value_before = self.is_present(self.locators.PROGRESS_BAR_VALUE).text
        progress_bar_button = self.is_clickable(self.locators.PROGRESS_BAR_BUTTON)
        progress_bar_button.click()
        time.sleep(random.randint(1, 5))
        progress_bar_button.click()
        value_after = self.is_present(self.locators.PROGRESS_BAR_VALUE).text
        return value_before, value_after
...
class TabsPage(BasePage):
    ...
    def check_tabs(self, name_tab):
        ...
        button = self.is_visible(tabs[name_tab]['title'])
        button.click()
        what_content = self.is_visible(tabs[name_tab]['content']).text
